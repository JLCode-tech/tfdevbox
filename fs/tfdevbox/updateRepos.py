...
def process_repo_file(filename, stack={"repos":[]}, depth=0):
	...
	if 'include' in repo_dict:
		for includerepo in repo_dict["include"]:
			...
			stack = process_repo_file("/tmp/tfdevbox-repo/images/%s/fs/etc/tfdevboxrepo.d/%s.json" % (includerepo, includerepo), stack, depth+1)
...
parser = argparse.ArgumentParser(description='Script to update tfdevbox container repo list')
...
# Here the script will process a file named user_repos.json which can be mounted to /tmp in the docker run command
user_repos = process_repo_file('/tmp/user_repos.json')

debug(json.dumps(repos, indent=1))

with open('/home/tfdevbox/repos.json', 'w') as out:
