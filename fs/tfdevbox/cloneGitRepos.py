...
parser.add_argument("-f", "--filename", default="/home/tfdevbox/repos.json", help="The JSON repo file")
...
log = safe_open('/home/tfdevbox/log/cloneGitRepos.log', 'a')
...
for i in range(0, numrepos):
	...
	if 'localdir' not in obj:
		obj["localdir"] = '/home/tfdevbox/%s' % obj["name"]
	...
	install_fn = '%s/tfdevbox_install.sh' % obj["localdir"]
	...
	if os.path.exists(install_fn):
		...
		exitcode = call(['/bin/bash','-c','%s/tfdevbox_install.sh' % obj["localdir"]], shell=False, stdout=install_log, stderr=install_log, cwd=obj["localdir"])
		install_log.close()
		if exitcode > 0:
...
done = safe_open('/tfdevbox/TFDEVBOX_ENV', 'a')
done.write('TFDEVBOX_CLONE_DONE=1\n')
done.close()